...
- [ ] 1.1.4 实现泡沫层效果
  - 泡沫颜色 `#F5E6D3`
  - SVG 波浪 path，8px 高度
- [ ] 1.1.5 实现蒸汽动画（进度 > 30% 时显示）
  - 3 条曲线，各 40px 高
  - 飘动动画 2s 循环，opacity 渐变
- [ ] 1.1.6 实现水位上涨动画（打卡成功时触发）
...
- [ ] 3.5.1 更新按钮颜色为 CTA 橙色 `#F97316`
- [ ] 3.5.2 保留现有打卡逻辑
- [ ] 3.5.3 集成 confetti 效果
...
### 4.2 动画效果
- [ ] 4.2.1 实现咖啡水位上升动画 (600ms, ease-out)
- [ ] 4.2.2 实现蒸汽飘动动画 (2s, linear, 循环)
- [ ] 4.2.3 实现奶油波浪动画 (1.5s, ease-in-out, 循环)
- [ ] 4.2.4 实现奶油滴落动画 (400ms, ease-in)
- [ ] 4.2.5 实现水波纹动画 (2s, ease-in-out, 循环)
- [ ] 4.2.6 实现冰块裂纹动画 (300ms, ease-out)
- [ ] 4.2.7 实现水滴滴落动画 (500ms, ease-in, 循环)

### 4.3 无障碍支持
- [ ] 4.3.1 实现 `prefers-reduced-motion` 媒体查询
  ```css
...
- [ ] 5.1 将 `CheckInHistoryPanel` 包装为全屏 Popup
- [ ] 5.2 将 `HistoryCyclePanel` 包装为全屏 Popup
- [ ] 5.3 实现从二级入口跳转逻辑
...
- [ ] 6.1.1 重构 `index.tsx`
- [ ] 6.1.2 集成所有新组件
- [ ] 6.1.3 保留现有 Modal（CheckInModal、RecordDataModal）
...
**P1 - 重要功能**：1.2, 1.3, 3.2, 3.3, 3.4, 4.1, 4.3
